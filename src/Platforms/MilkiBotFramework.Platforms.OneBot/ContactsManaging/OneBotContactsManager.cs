...
public sealed class OneBotContactsManager : ContactsManagerBase
{
    ...
    public override async Task<SelfInfoResult> TryGetOrUpdateSelfInfo()
    {
        ...
        var result = await _oneBotApi.GetLoginInfo();
    ...
    public override async Task<ChannelInfoResult> TryGetOrAddChannelInfo(string channelId, string? subChannelId = null)
    {
        ...
        if (subChannelId == null)
        {
            try
            {
                var groupInfo = await _oneBotApi.GetGroupInfo(long.Parse(channelId));
    ...
    public override async Task<MemberInfoResult> TryGetOrAddMemberInfo(string channelId, string userId, string? subChannelId = null)
    {
        ...
        try
        {
            groupMember = await _oneBotApi.GetGroupMemberDetail(long.Parse(channelId), long.Parse(userId));
        }
        catch (OneBotApiException ex)
        {
    ...
    public override async Task<PrivateInfoResult> TryGetOrAddPrivateInfo(string userId)
    {
        ...
        try
        {
            var stranger = await _oneBotApi.GetStrangerInfo(long.Parse(userId));
    ...
    {
        var friends = _oneBotApi.GetFriends().Result;
        ...
        var allGroups = _oneBotApi.GetGroups().Result;
        foreach (var groupInfo in allGroups)
        {
            var allMembers = _oneBotApi.GetFuzzyGroupMembers(Convert.ToInt64(groupInfo.GroupId)).Result;
